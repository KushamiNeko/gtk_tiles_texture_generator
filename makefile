...
bin/pattern_control: bin/pattern_model 
	gcc `pkg-config --cflags gtk+-3.0 glew` -c src/pattern_control.c \
`pkg-config --libs gtk+-3.0 glew` -o bin/pattern_control 


bin/pattern_model: bin/rectangle 
	gcc `pkg-config --cflags gtk+-3.0 glew` -c src/pattern_model.c -o bin/pattern_model \
		`pkg-config --libs gtk+-3.0 glew`

bin/rectangle:
	gcc -c src/rectangle.c -o bin/rectangle
